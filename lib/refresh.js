...
const linkTheme = (dir) => {
  const src = path.join('src', 'themes', path.basename(dir), path.dirname(dir));
  const dest = path.relative(path.dirname(src), path.join('themes', path.basename(dir), path.dirname(dir)));
  return symlink(dest, src).catch(err => '');
};

const linkPlugin = (dir) => {
  const src = path.join('src', 'custom', 'plugins', path.basename(dir), path.dirname(dir));
  const dest = path.relative(path.dirname(src), path.join('plugins', path.basename(dir), path.dirname(dir)));
  return symlink(dest, src).catch(err => '');
};

const linkLegacyPlugin = (dir) => {
  const base = path.join('src', 'engine', 'Shopware', 'Plugins', 'Local');
  const src = path.join(base, path.basename(dir), path.dirname(dir));
  const dest = path.relative(path.dirname(src), path.join('plugins-legacy', path.basename(dir), path.dirname(dir)));
  return symlink(dest, src).catch(err => '');
};
        ...
        return new Listr([
          ...
          },
          {
            ...
            task: () => Promise.all([
              ...
              globby('*/*/', {cwd: dir('plugins-legacy')}).then((plugins) => {
